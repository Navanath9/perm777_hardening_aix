...
process_one() {
  ...
  if [[ -n "$_FSL" ]]; then
    ...
    # Fallback: individual calls (perl unavailable)
    _OCT=$(get_octal "$_P")
    typeset _OG
    _OG=$(get_og "$_P")
  ...
  if [[ "$_TYPE" = "directory" ]]; then
    if [[ $SKIP_REC -eq 0 ]]; then
      ...
      # Fast count: immediate children only via ls
      # Accurate for showing "how many files are directly in this folder"
      # without recursing into potentially huge subtrees
      _RC=$(ls -1a "$_P" 2>/dev/null | awk 'END{print NR>2 ? NR-2 : 0}')
      # ls -1a includes . and .. so subtract 2
      # If ls fails or returns nothing treat as 0
      is_int "$_RC" || _RC=0
    else
...
run_discovery() {
  ...
  #    N  = immediate child count (COUNT_MATCHED_DIRS=1, fast)
  #    N  = full recursive count  (SKIP_REC=0, slow)
  #    1  = file (not a directory)
